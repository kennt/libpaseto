...
// basic use case
TEST_CASE("paseto_v4secret_basic", "[paseto_v4secret]")
{
    auto [ public_key, secret_key ] =
        paseto::KeyGen::generatePair(paseto::KeyType::V4_PUBLIC);

    REQUIRE( secret_key->keyType() == paseto::KeyType::V4_SECRET );
    REQUIRE( secret_key->size() == paseto_v4_PUBLIC_SECRETKEYBYTES );
    REQUIRE( secret_key->required_length() == paseto_v4_PUBLIC_SECRETKEYBYTES );
...
TEST_CASE("paseto_v4secret_unsupported_apis", "[paseto_v4secret]")
{
    auto [ public_key, secret_key ] =
        paseto::KeyGen::generatePair(paseto::KeyType::V4_PUBLIC);
    ...
    REQUIRE_THROWS( paseto::KeyGen::generate(paseto::KeyType::V4_SECRET) );
}


TEST_CASE("paseto_v4secret_implicitassertion", "[paseto_v4secret]")
{
    auto [ public_key, secret_key ] =
        paseto::KeyGen::generatePair(paseto::KeyType::V4_PUBLIC);
...
TEST_CASE("paseto_v4secret_loadFrom", "[paseto_v4secret]")
{
    string data {"test data"};

    // Test the various loadFrom calls
    uint8_t binary_data[paseto_v4_PUBLIC_SECRETKEYBYTES+1];
    randombytes_buf(binary_data, sizeof(binary_data));

    // loadFromBinary
    {
        auto key = paseto::Keys::loadFromBinary(paseto::KeyType::V4_SECRET,
           paseto::binary_view(binary_data, paseto_v4_PUBLIC_SECRETKEYBYTES));

        // length-testing
        for (size_t i=0; i<paseto_v4_PUBLIC_SECRETKEYBYTES; i++)
        {
    ...
    // loadFromHex
    {
        ...
        auto key = paseto::Keys::loadFromHex(paseto::KeyType::V4_SECRET,
                    string_view(hex_data, 2*paseto_v4_PUBLIC_SECRETKEYBYTES));
    ...
    // loadFromBase64
    {
        ...
        REQUIRE( save_base64(base64_data, sizeof(base64_data), &len,
                       binary_data, paseto_v4_PUBLIC_SECRETKEYBYTES) );
        auto key = paseto::Keys::loadFromBase64(paseto::KeyType::V4_SECRET,
...
// Test unsupported Paserk APIs on V4_SECRET keys
TEST_CASE("paseto_v4secret_lucidity", "[paseto_v4secret]")
{
    auto [ public_key, secret_key ] =
        paseto::KeyGen::generatePair(paseto::KeyType::V4_PUBLIC);
...
TEST_CASE("paseto_v4secret_getPublicKey", "[paseto_v4secret]")
{
    auto [ public_key, secret_key ] =
        paseto::KeyGen::generatePair(paseto::KeyType::V4_PUBLIC);
    ...
    REQUIRE( key->keyType() == paseto::KeyType::V4_PUBLIC );
