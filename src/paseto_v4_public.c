    ...
    const uint8_t * secret, size_t secret_len)
{
    if (strncmp(paserk_key, paserk_public, paserk_public_len) == 0)
    {
        ...
        {
            if (len != paseto_v4_PUBLIC_PUBLICKEYBYTES)
            {
                fprintf(stderr, "unexpected key length: actual:%zu expected:%u\n",
                    len, paseto_v4_PUBLIC_PUBLICKEYBYTES);
                sodium_memzero(key, paseto_v4_PUBLIC_PUBLICKEYBYTES);
                errno = EINVAL;
                return false;
            }
            return true;
    ...
    const uint8_t * secret, size_t secret_len)
{
    if (strncmp(paserk_key, paserk_secret, paserk_secret_len) == 0)
    {
        ...
        if (sodium_base642bin(
                ...
                sodium_base64_VARIANT_URLSAFE_NO_PADDING) == 0)
        {
            if (len != paseto_v4_PUBLIC_SECRETKEYBYTES)
            {
                fprintf(stderr, "unexpected key length: actual:%zu expected:%u\n",
                    len, paseto_v4_PUBLIC_SECRETKEYBYTES);
                sodium_memzero(key, paseto_v4_PUBLIC_SECRETKEYBYTES);
                errno = EINVAL;
                return false;
            }
            return true;
